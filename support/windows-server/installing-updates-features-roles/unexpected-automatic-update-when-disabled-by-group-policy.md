...
This issue occurs during group a policy processing event. This command resets all policy settings and reapplies them. During this interval, the automatic update settings are reverted to defaults, and updates might be installed. This issue occurs only on Windows Server 2016 and Windows Server 2019.

## Resolution

To avoid this issue, restrict gorup policy processing to only if changes to group policy object have been made on Windows Server 2016 and 2019 unless you're prepared for any queued updates to be installed. This issue is rare, but it can occur.
